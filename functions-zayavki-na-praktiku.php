...
function checkIsTeacher($connect,$user_id){
		$groups_query = $connect->query("SELECT GROUP_ID FROM b_user_group WHERE USER_ID = '$user_id';");
		$groups=array();
...
/*======================== СКАЧАТЬ ФАЙЛ =========================*/
   ...
   function Download_Otchet($link){
	  $file =  "../../../../studentu/praktika/vybor-prepodavatelya-na-praktiku/".$link;
	  $filename = str_replace(['uploads/'], "", $link);
	  if(!file_exists($file)){
			  die('file not found');
...
/*======================== НАГРУЗКА НА ПРЕПОДОВАТЕЛЯ МИНУС =========================*/

function work_load_decriment($connect, $teacher_id){
      try{
         $resultset=$connect->query("SELECT work_load FROM Practices.teachers Where id = '".$_POST['teacher_id']."'");
      }
      catch(Exception $e){
...
/*======================== НАГРУЗКА НА ПРЕПОДОВАТЕЛЯ ПЛЮС =========================*/

function work_load_increment($connect, $teacher_id){
      try{
         $resultset=$connect->query("SELECT work_load FROM Practices.teachers Where id = '".$_POST['teacher_id']."'");
      }
      catch(Exception $e){
